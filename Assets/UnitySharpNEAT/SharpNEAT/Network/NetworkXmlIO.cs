...
using System.IO;
using System.Xml;
using UnitySharpNEAT.SharpNEAT.Network.ActivationFunctions;
using UnitySharpNEAT.SharpNEAT.Network.ActivationFunctions.Bipolar;
using UnitySharpNEAT.SharpNEAT.Network.ActivationFunctions.RadialBasis;
using UnitySharpNEAT.SharpNEAT.Network.ActivationFunctions.Unipolar;
using UnitySharpNEAT.SharpNEAT.Utility;

namespace UnitySharpNEAT.SharpNEAT.Network
{
    /// <summary>
    ...
    public static class NetworkXmlIO
    {
        ...
        private const string __ElemNetworks = "Networks";
        private const string __ElemNetwork = "Network";
        private const string __ElemNodes = "Nodes";
        private const string __ElemNode = "Node";
        private const string __ElemConnections = "Connections";
        private const string __ElemConnection = "Con";
        private const string __ElemActivationFunctions = "ActivationFunctions";
        private const string __ElemActivationFn = "Fn";

        private const string __AttrId = "id";
        private const string __AttrName = "name";
        private const string __AttrType = "type";
        private const string __AttrSourceId = "src";
        private const string __AttrTargetId = "tgt";
        private const string __AttrWeight = "wght";
        private const string __AttrActivationFunctionId = "fnId";
        private const string __AttrAuxState = "aux";
        private const string __AttrProbability = "prob";
        ...
        {
            using(var xr = new XmlNodeReader(xmlNode))
            {
        ...
        {
            var count = networkDefList.Count;
            var tmpList = new List<INetworkDefinition>(count);
            foreach(var networkDef in networkDefList) {
        ...
        public static void WriteComplete(XmlWriter xw, IList<INetworkDefinition> networkDefList, bool nodeFnIds)
        {
            ...
            var activationFnLib = networkDefList[0].ActivationFnLibrary;
        ...
        public static void Write(XmlWriter xw, INetworkDefinition networkDef, bool nodeFnIds)
        {
            ...
            foreach(var con in networkDef.ConnectionList)
            {
        ...
        public static void Write(XmlWriter xw, IActivationFunctionLibrary activationFnLib)
        {
            ...
            var fnList = activationFnLib.GetFunctionList();
        ...
        public static List<NetworkDefinition> ReadCompleteNetworkDefinitionList(XmlReader xr, bool nodeFnIds)
        {
            ...
            var activationFnLib = ReadActivationFunctionLibrary(xr);
            XmlIoUtils.MoveToElement(xr, false, __ElemNetworks);

            var networkDefList = new List<NetworkDefinition>();
            using(var xrSubtree = xr.ReadSubtree())
            {
                ...
                do
                {
                    var networkDef = ReadNetworkDefinition(xr, activationFnLib, nodeFnIds);
        ...
        public static NetworkDefinition ReadNetworkDefinition(XmlReader xr, IActivationFunctionLibrary activationFnLib, bool nodeFnIds)
        {
            ...
            using(var xrSubtree = xr.ReadSubtree())
            {
                ...
                do
                {
                    var nodeType = ReadAttributeAsNodeType(xrSubtree, __AttrType);
                    var id = XmlIoUtils.ReadAttributeAsUInt(xrSubtree, __AttrId);
                    var fnId = 0;
                    ...
                    var node = new NetworkNode(id, nodeType, fnId, auxState);
                    nodeList.Add(node);
            ...
            using(var xrSubtree = xr.ReadSubtree())
            {
                ...
                {   // We have at least one connection.
                    ...
                    do
                    {
                        var srcId = XmlIoUtils.ReadAttributeAsUInt(xrSubtree, __AttrSourceId);
                        var tgtId = XmlIoUtils.ReadAttributeAsUInt(xrSubtree, __AttrTargetId);
                        var weight = XmlIoUtils.ReadAttributeAsDouble(xrSubtree, __AttrWeight);
                        var conn = new NetworkConnection(srcId, tgtId, weight);
                        connList.Add(conn);
                    } 
        ...
        public static IActivationFunctionLibrary ReadActivationFunctionLibrary(XmlReader xr)
        {
            ...
            var fnList = new List<ActivationFunctionInfo>();
            using(var xrSubtree = xr.ReadSubtree())
            {
                ...
                do
                {
                    var id = XmlIoUtils.ReadAttributeAsInt(xrSubtree, __AttrId);
                    var selectionProb = XmlIoUtils.ReadAttributeAsDouble(xrSubtree, __AttrProbability);
                    var fnName = xrSubtree.GetAttribute(__AttrName);

                    // Lookup function name.
                    var activationFn = GetActivationFunction(fnName);

                    // Add new function to our list of functions.
                    var fnInfo = new ActivationFunctionInfo(id, selectionProb, activationFn);
                    fnList.Add(fnInfo);
                }
