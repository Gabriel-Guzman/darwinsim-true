...
namespace UnitySharpNEAT.SharpNEAT.Network
{
    ...
    public class NetworkDefinition : INetworkDefinition
    {
        ...
        private readonly bool _isAcyclic = false;
        private readonly IActivationFunctionLibrary _activationFnLib;
        ...
        public NetworkConnectivityData GetConnectivityData()
        {
            ...
            {
                var nodeId = _nodeList[i].Id;
                var ncd = new NodeConnectivityData(nodeId);
            ...
            var conCount = _connectionList.Count;
            for(var i=0; i<conCount; i++)
            {
                var conn = _connectionList[i];
                var srcNodeData = nodeConnectivityDataById[conn.SourceNodeId];
                var tgtNodeData = nodeConnectivityDataById[conn.TargetNodeId];
        ...
        public bool PerformIntegrityCheck()
        {
            ...
            var prevId = 0u;
            var idx = 1;
            for(var i=0; i<_inputNodeCount; i++, idx++)
            {
            ...
            for(var i=0; i<_outputNodeCount; i++, idx++)
            {
            ...
            {
                var key = new ConnectionEndpointsStruct(conn.SourceNodeId, conn.TargetNodeId);
