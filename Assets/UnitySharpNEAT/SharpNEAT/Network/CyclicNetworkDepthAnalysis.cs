...
namespace UnitySharpNEAT.SharpNEAT.Network
{
    ...
    public class CyclicNetworkDepthAnalysis
    {
        ...
        /// </summary>
        private readonly Dictionary<uint,int> _nodeDepthById = new Dictionary<uint,int>();
        ...
        public NetworkDepthInfo CalculateNodeDepths(INetworkDefinition networkDef)
        {
            ...
            var visitedNodeSet = new HashSet<uint>();
            var inputAndBiasCount = networkDef.InputNodeCount + 1;
            for(var i=0; i<inputAndBiasCount; i++)
            {
        ...
        private void TraverseNode(NodeConnectivityData nodeData, HashSet<uint> visitedNodeSet, int depth)
        {
            ...
            foreach(var targetId in nodeData._tgtNodes)
            {
