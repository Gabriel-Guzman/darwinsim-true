...
namespace UnitySharpNEAT.SharpNEAT.Utility
{
    /// <summary>
    ...
    ///     - Added new statically declared seedRng FastRandom to allow easy creation of multiple FastRandoms with different
    ///     seeds
    ...
    public class FastRandom
    {
        ...
        /// </summary>
        private static readonly FastRandom __seedRng = new(Environment.TickCount);
        ...
        public int Next(int lowerBound, int upperBound)
        {
            ...
            if (range < 0)
                ...
                return lowerBound + (int)(REAL_UNIT_UINT * (_w = _w ^ (_w >> 19) ^ t ^ (t >> 8)) *
                                          (upperBound - (long)lowerBound));

            // 31 bits of precision will suffice if range<=int.MaxValue. This allows us to cast to an int and gain
            // a little more performance.
            return lowerBound +
                   (int)(REAL_UNIT_INT * (int)(0x7FFFFFFF & (_w = _w ^ (_w >> 19) ^ t ^ (t >> 8))) * range);
        }

        /// <summary>
        ///     Generates a random double. Values returned are over the range [0, 1). That is, inclusive of 0.0 and exclusive of
        ///     1.0.
        /// </summary>
