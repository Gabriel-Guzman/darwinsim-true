...
namespace UnitySharpNEAT.SharpNEAT.DistanceMetrics
{
    ...
    public class DistanceMetricUtils
    {
        ...
        public static CoordinateVector CalculateCentroid(IDistanceMetric distanceMetric, IList<CoordinateVector> coordList)
        {
            ...
            var centroidIdx = 0;
            var centroidDistance = CalculateMeanDistanceFromCoords(distanceMetric, coordList, 0);
            var count = coordList.Count;
            for(var i=1; i<count; i++)
            {
                var distance = CalculateMeanDistanceFromCoords(distanceMetric, coordList, i);
