...
namespace UnitySharpNEAT.SharpNEAT.DistanceMetrics
{
    ...
    public class EuclideanDistanceMetric : IDistanceMetric
    {
        ...
        public bool MeasureDistance(CoordinateVector p1, CoordinateVector p2, double threshold)
        {
            ...
            var arr1 = p1.CoordArray;
            var arr2 = p2.CoordArray;
        ...
        //      before the threshold is passed and we exit the method.
            var arr1Idx = arr1Length - 1;
            var arr2Idx = arr2Length - 1;
            var elem1 = arr1[arr1Idx];
            var elem2 = arr2[arr2Idx];
            for(;;)
            {
                ...
                else if(elem1.Key == elem2.Key)
                {
                    ...
                    var tmp = elem1.Value - elem2.Value;
                    distance += tmp * tmp;
        ...
        //----- Both arrays contain elements. 
            ...
            for(;;)
            {
                ...
                {   // All remaining arr2 elements are mismatches.
                    for(var i=arr2Idx; i<arr2Length; i++) {
                        distance += arr2[i].Value * arr2[i].Value;
                    }
                    return Math.Sqrt(distance);
                }
                ...
                {   // All remaining arr1 elements are mismatches.
                    for(var i=arr1Idx; i<arr1.Length; i++) {
                        distance += arr1[i].Value * arr1[i].Value;
                    }
                    return Math.Sqrt(distance);
                }
        ...
        public CoordinateVector CalculateCentroid(IList<CoordinateVector> coordList)
        {
            ...
            var coordElemTotals = new SortedDictionary<ulong,double[]>();

            // Loop over coords.
            foreach(var coord in coordList)
            {
                // Loop over each element within the current coord.
                foreach(var coordElem in coord.CoordArray)
                {
            ...
            var coordCountReciprocol = 1.0 / (double)coordList.Count;
            var centroidElemArr = new KeyValuePair<ulong,double>[coordElemTotals.Count];
            var i=0;
            foreach(var coordElem in coordElemTotals)
