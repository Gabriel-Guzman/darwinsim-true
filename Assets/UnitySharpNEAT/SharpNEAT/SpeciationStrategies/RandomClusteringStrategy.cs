...
namespace UnitySharpNEAT.SharpNEAT.SpeciationStrategies
{
        ...
        where TGenome : class, IGenome<TGenome>
    {
        ...
        public IList<Specie<TGenome>> InitializeSpeciation(IList<TGenome> genomeList, int specieCount)
        {
            ...
            var capacity = (int)Math.Ceiling((double)genomeList.Count/(double)specieCount);
        ...
        public void SpeciateGenomes(IList<TGenome> genomeList, IList<Specie<TGenome>> specieList)
        {
            ...
            var genomesPerSpecie = genomeCount / specieCount;
            ...
            for(var i=0; i<specieCount; i++)
            {
                ...
                for(var j=0; j<genomesPerSpecie; j++, genomeIdx++) 
                {
            ...
            var specieIdxArr = new int[specieCount];
            for(var i=0; i<specieCount; i++) {
                specieIdxArr[i] = i;
            }
            Utilities.Shuffle(specieIdxArr, _rng);

            for(var i=0; i<specieCount && genomeIdx < genomeCount; i++, genomeIdx++) 
            {
                var specieIdx = specieIdxArr[i];
        ...
        public void SpeciateOffspring(IList<TGenome> genomeList, IList<Specie<TGenome>> specieList)
        {
            ...
            var genomeCount = gList.Count;
            var totalGenomeCount = genomeCount;
            ...
            // This is guaranteed to be at least 1 because genomeCount >= specieCount.
            var specieCount = specieList.Count;
            var genomesPerSpecie = totalGenomeCount / specieCount;
            ...
            var sList = new List<Specie<TGenome>>(specieList);
            ...
            var genomeIdx = 0;
            for(var i=0; i<specieCount && genomeIdx<genomeCount; i++)
            {
                var specie = sList[i];
                var fillcount = genomesPerSpecie - specie.GenomeList.Count;
