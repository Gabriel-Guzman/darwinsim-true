...
namespace UnitySharpNEAT.SharpNEAT.SpeciationStrategies
{
        ...
        where TGenome : class, IGenome<TGenome>
    {
        ...
        public IList<Specie<TGenome>> InitializeSpeciation(IList<TGenome> genomeList, int specieCount)
        {
            ...
            var initSpeciesCapacity = (genomeList.Count * 2) / specieCount;
            var specieList = new List<Specie<TGenome>>(specieCount);
        ...
        public void SpeciateGenomes(IList<TGenome> genomeList, IList<Specie<TGenome>> specieList)
        {
            ...
            var genomeCount = genomeList.Count;
            for(var i=specieCount; i<genomeCount; i++) {
        ...
        public void SpeciateOffspring(IList<TGenome> offspringList, IList<Specie<TGenome>> specieList)
        {
            ...
            foreach(var genome in offspringList)
            {
                var closestSpecie = FindClosestSpecie(genome, specieList);
                closestSpecie.GenomeList.Add(genome);
                genome.SpecieIdx = closestSpecie.Idx;
        ...
        {
            var emptySpecieList = new List<Specie<TGenome>>();
            var specieCount = specieList.Count;

            // Array of flags that indicate if a specie was modified (had genomes allocated to and/or from it).
            var specieModArr = new bool[specieCount];

            // Main k-means loop.
            for(var loops=0; loops<__MAX_KMEANS_LOOPS; loops++)
            {
                ...
                if(0 != emptySpecieList.Count)
                {
                    ...
                    var emptySpecieCount = emptySpecieList.Count;
                    var outlierIdx = 0;
                    for(var i=0; i<emptySpecieCount; i++)
                    {
        ...
        private TGenome[] GetGenomesByDistanceFromSpecie(IList<TGenome> genomeList, IList<Specie<TGenome>> specieList)
        {
            ...
            {
                var genome = genomeList[i];
                var distance = _distanceMetric.MeasureDistance(genome.Position, specieList[genome.SpecieIdx].Centroid);
        ...
        private Specie<TGenome> FindClosestSpecie(TGenome genome, IList<Specie<TGenome>> specieList)
        {
            ...
            var closestDistance = _distanceMetric.MeasureDistance(genome.Position, closestSpecie.Centroid);

            // Measure distance to all remaining species.
            var speciesCount = specieList.Count;
            for(var i=1; i<speciesCount; i++)
            {
                var distance = _distanceMetric.MeasureDistance(genome.Position, specieList[i].Centroid);
