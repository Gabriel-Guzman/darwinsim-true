...
namespace UnitySharpNEAT.SharpNEAT.Phenomes.NeuralNets.CyclicNetwork
{
    /// <summary>
    ...
    public class RelaxingCyclicNetwork : CyclicNetwork
    {
        ...
        private readonly double _signalDeltaThreshold;
        ...
        public override void Activate()
        {
            ...
            var connectionCount = _connectionList.Count;
            var neuronCount = _neuronList.Count;
            var isNotRelaxed = true;
            for(var i=0; i<_timestepsPerActivation && isNotRelaxed; i++)
            {
                isNotRelaxed = false;
                ...
                {
                    var connection = _connectionList[j];
                ...
                // fixed output value.
                for(var j=_inputAndBiasNeuronCount; j<neuronCount; j++) 
                {
                    var neuron = _neuronList[j];
                    var tmp = neuron.ActivationFunction.Calculate(neuron.InputValue, neuron.AuxiliaryArguments);
