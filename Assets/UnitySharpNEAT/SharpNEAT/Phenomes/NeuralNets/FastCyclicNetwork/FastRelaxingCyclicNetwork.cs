...
namespace UnitySharpNEAT.SharpNEAT.Phenomes.NeuralNets.FastCyclicNetwork
{
    /// <summary>
    ...
    public class FastRelaxingCyclicNetwork : FastCyclicNetwork
    {
        ...
        public override void Activate()
        {
            ...
            for(var i=0; i<_timestepsPerActivation && isNotRelaxed; i++)
            {
                ...
                for(var j=0; j<_connectionArray.Length; j++) {
                ...
                for(var j=_inputAndBiasNeuronCount; j<_preActivationArray.Length; j++)
                {
                    var tmp = _neuronActivationFnArray[j].Calculate(_preActivationArray[j], _neuronAuxArgsArray[j]);
