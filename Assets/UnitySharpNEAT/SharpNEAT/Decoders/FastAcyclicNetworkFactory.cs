...
using UnitySharpNEAT.SharpNEAT.Phenomes.NeuralNets.FastCyclicNetwork;

namespace UnitySharpNEAT.SharpNEAT.Decoders
{
    /// <summary>
    ...
    public class FastAcyclicNetworkFactory
    {
        ...
        public static FastAcyclicNetwork CreateFastAcyclicNetwork(INetworkDefinition networkDef)
        {
            ...
            var depthAnalysis = new AcyclicNetworkDepthAnalysis();
            var netDepthInfo = depthAnalysis.CalculateNodeDepths(networkDef);

            // Construct an array of NodeInfo, ordered by node depth.
            // Create/populate NodeInfo array.
            var nodeDepthArr = netDepthInfo._nodeDepthArr;
            var nodeList = networkDef.NodeList;
            var nodeCount = nodeList.Count;
            var nodeInfoByDepth = new NodeInfo[nodeCount];
            ...
            var inputAndBiasCount = networkDef.InputNodeCount + 1;
            ...
            var newIdxByDefinitionIdx = new int[nodeCount];
            ...
            var newIdxById = new Dictionary<uint,int>(nodeCount);

            // Populate both the lookup array and dictionary.
            for(var i=0; i<nodeCount; i++) 
            {
                var nodeInfo = nodeInfoByDepth[i];
            ...
            var outputCount = networkDef.OutputNodeCount;
            var outputNeuronIdxArr = new int[outputCount];
            ...
            var activationFnLibrary = networkDef.ActivationFnLibrary;
            var nodeActivationFnArr = new IActivationFunction[nodeCount];
            var nodeAuxArgsArray = new double[nodeCount][];
            for(var i=0; i<nodeCount; i++) 
            {
                var definitionIdx = newIdxByDefinitionIdx[i];
        ...
        //=== Create array of FastConnection(s). 
            ...
            var connectionList = networkDef.ConnectionList;
            var connectionCount = connectionList.Count;
            var fastConnectionArray = new FastConnection[connectionCount];
            ...
            var netDepth = netDepthInfo._networkDepth;
            var layerInfoArr = new LayerInfo[netDepth];
            ...
            var nodeIdx = inputAndBiasCount;
            var connIdx = 0;

            for(var currDepth=0; currDepth < netDepth; currDepth++)
            {
