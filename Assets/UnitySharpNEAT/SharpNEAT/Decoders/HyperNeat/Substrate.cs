...
namespace UnitySharpNEAT.SharpNEAT.Decoders.HyperNeat
{
    ...
    public class Substrate
    {
                         ...
                         List<NodeSetMapping> nodeSetMappingList)
        {
            ...
            var nonBiasConnectionCountHint = 0;
            foreach(var mapping in nodeSetMappingList) {
            ...
            {   
                ...
                foreach(var mapping in nodeSetMappingList)
                {
                    var connectionSequence = mapping.GenerateConnections(nodeSetList);
                    foreach(var conn in connectionSequence) {
        ...
        public INetworkDefinition CreateNetworkDefinition(IBlackBox blackbox, bool lengthCppnInput)
        {
            ...
            // generated sequence.
            var connectionSequence = _connectionList ?? GetConnectionSequence();

            // Iterate over substrate connections. Determine each connection's weight and create a list
            // of network definition connections.
            var inputSignalArr = blackbox.InputSignalArray;
            var outputSignalArr = blackbox.OutputSignalArray;
            var networkConnList = new ConnectionList(_connectionCountHint);
            var lengthInputIdx = _dimensionality + _dimensionality;

            foreach(var substrateConn in connectionSequence)
            {
                ...
                for(var i=0; i<_dimensionality; i++) 
                {
            ...
            var setCount = _nodeSetList.Count;
            for(var i=1; i<setCount; i++)
            {
                var nodeSet = _nodeSetList[i];
                foreach(var node in nodeSet.NodeList)
                {
                    ...
                    for(var j=0; j<_dimensionality; j++)
                    {
            ...
            var networkDef = new NetworkDefinition(_inputNodeCount, _outputNodeCount,
        ...
        private void VaildateSubstrateNodes(List<SubstrateNodeSet> nodeSetList)
        {
            ...
            var idDict = new Dictionary<uint,object>();
            ...
            var inputNodeSet = nodeSetList[0];
            var count = inputNodeSet.NodeList.Count;
            var expectedId = 1;
            for(var i=0; i<count; i++, expectedId++)
            {
            ...
            var outputNodeSet = nodeSetList[1];
            ...
            var setCount = nodeSetList.Count;
            for(var i=2; i<setCount; i++)
            {
                var hiddenNodeSet = nodeSetList[i];
                foreach(var node in hiddenNodeSet.NodeList)
                {
        ...
        private NodeList CreateNetworkNodeList()
        {
            ...
            var nodeCount = 0;
            foreach(var set in _nodeSetList) {
            ...
            var nodeList = new NodeList(nodeCount);
            ...
            var nodeSet = _nodeSetList[0];
            var setNodeCount = nodeSet.NodeList.Count;

            // Create input nodes.
            for(var i=0; i<setNodeCount; i++) {
            ...
            var setCount = _nodeSetList.Count;
            for(var nodeSetIdx=2; nodeSetIdx<setCount; nodeSetIdx++)
            {
        ...
        private IEnumerable<SubstrateConnection> GetConnectionSequence()
        {
            ...
            var count = _nodeSetMappingList.Count;
            if(0 == count)
            {   // No mappings.
                return null;
            }
            ...
            var enumerable = _nodeSetMappingList[0].GenerateConnections(_nodeSetList);
